...
public class BloblessCloneApiTests
{
    ...
    public async Task FetchBloblessClone_returns_commits_and_trees_without_blobs()
    {
        ...
        // Act
        var repository =
            await LoadFromUrl.FetchBloblessCloneAsync(repositoryUrl, commitSha);
        ...
        var blobCount =
            repository.Objects.Values.Count(obj => obj.Type is PackFile.ObjectType.Blob);
        ...
        {
            (obj.Type is PackFile.ObjectType.Commit || obj.Type is PackFile.ObjectType.Tree)
    ...
    public async Task FetchBloblessClone_with_depth_parameter()
    {
        ...
        var commitCount =
            repository.Objects.Values.Count(obj => obj.Type is PackFile.ObjectType.Commit);
    ...
    public void FetchBloblessClone_synchronous_version()
    {
        ...
        // Act
        var repository =
            LoadFromUrl.FetchBloblessClone(repositoryUrl, commitSha);
    ...
    public async Task Complete_workflow_navigate_and_list_entries()
    {
        ...
        var files =
            entries.Where(e => e.Mode.StartsWith("100")).ToList();

        var directories =
            entries.Where(e => e.Mode is "40000").ToList();
    ...
    public async Task Navigate_commit_parent_chain_and_verify_commit_metadata()
    {
        ...
        var repository =
            await LoadFromUrl.FetchBloblessCloneAsync(repositoryUrl, startCommitSha, depth: null);
        ...
        for (var i = 0; i < 4; i++)
        {
            ...
            commit.Author.Timestamp.Should().NotBe(default, $"Commit {i} author should have a valid timestamp");
            ...
            commit.Committer.Timestamp.Should().NotBe(default, $"Commit {i} committer should have a valid timestamp");
