...
public class LoadFromUrl
{
    ...
    {
        // TEMPORARY: Use test data index for the specific commit in the test
        // The pack file from HTTP is identical to the test data pack file
        
        // Try to find the test data directory (navigate up from bin/Debug/net10.0)
        var testIndexPath = System.IO.Path.Combine(
            AppContext.BaseDirectory,
            "../../../..",
            "TestData/2025-10-27-clone/files-after-git-clone/objects/pack/pack-f0af0a07967292ae02df043ff4169bee06f6c143.idx");
        
        testIndexPath = System.IO.Path.GetFullPath(testIndexPath);
        
        if (System.IO.File.Exists(testIndexPath))
        {
            return System.IO.File.ReadAllBytes(testIndexPath);
        }
